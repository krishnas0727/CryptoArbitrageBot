...
def get_portfolio():
    ...
    if row:
        d = dict(row)
        bin_usdt = float(d.get("binance_usdt") or 4.63)
        byb_usdt = float(d.get("bybit_usdt") or 4.73)
        d["binance_usdt"] = bin_usdt
        d["bybit_usdt"] = byb_usdt
        d["usdt_balance"] = round(bin_usdt + byb_usdt, 2)
        return d
